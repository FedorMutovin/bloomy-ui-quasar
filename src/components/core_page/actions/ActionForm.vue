<template>
  <FormCard
    ...
    @submit="submitForm"
    :loading="loading"
    :header="$t(`forms.actions.create.title`)"
  >
    <template #form-fields>
      ...
      <q-card-section class="row">
        <EventTypeSelect class="col-12 col-md-7" event-type="action" @update:selected-event="handleSelectedType"/>
      </q-card-section>

      <q-separator spaced inset />

      <ActionFields @updateLocalEvent="updateLocalEvent"/>
    </template>
  </FormCard>
</template>
<script setup>
...
async function submitForm () {
  try {
    ...
    await actionStore.create(localRoot)
    $q.notify({
      type: 'positive',
