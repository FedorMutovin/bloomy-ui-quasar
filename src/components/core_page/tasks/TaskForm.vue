<template>
  <FormCard
    ...
    @submit="submitForm"
    :loading="loading"
    :header="$t(`forms.tasks.create.title`)"
  >
    <template #form-fields>
      ...
      <q-card-section class="row">
        <EventTypeSelect class="col-12 col-md-7" event-type="task" @update:selected-event="handleSelectedType"/>
      </q-card-section>

      <q-separator spaced inset />

      <TaskFields @updateLocalEvent="updateLocalEvent"/>
    </template>
  </FormCard>
</template>
<script setup>
...
async function submitForm () {
  try {
    ...
    await taskStore.create(localRoot)
    $q.notify({
      type: 'positive',
