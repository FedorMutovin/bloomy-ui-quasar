<template>
  <FormCard
    ...
    @submit="submitForm"
    :loading="loading"
    :header="$t(`forms.travels.create.title`)"
  >
    <template #form-fields>
      ...
      <q-card-section class="row">
        <RootTypeSelect class="col-12 col-md-7" root-type="travel" @update:selected-root="handleSelectedType"/>
      </q-card-section>

      <q-separator spaced inset />

      <TravelFields @updateLocalRoot="updateLocalRoot"/>
    </template>
  </FormCard>
</template>
<script setup>
...
import TravelFields from 'components/core_page/travels/forms/TravelFields.vue'
...
async function submitForm () {
  try {
    ...
    await travelStore.create(localRoot)
    $q.notify({
      type: 'positive',
